...
public class PersistenceConfiguration {
    ...
    CallLifecycleObserver callLifecycleObserver(VoiceCallCoordinator coordinator,ReactiveCallStore store,
        @Value("${voice.default-tenant-id:}") String tenant,@Value("${voice.default-assistant-id:}") String assistant){
        return new PlatformCallLifecycleObserver(coordinator,store,uuid(tenant),uuid(assistant));
    }
