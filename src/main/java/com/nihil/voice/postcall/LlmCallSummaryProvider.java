...
public final class LlmCallSummaryProvider implements CallSummaryProvider {
 ...
 public Mono<CallSummary> summarize(PostCallCandidate call){
  String system="Return only one JSON object with fields shortSummary, fullSummary, topic, intent, sentiment, leadQuality, outcome, nextAction, extractedData. topic is a short human-readable subject of the call. Use null when unknown. Do not wrap JSON in markdown.";
 ...
 @SuppressWarnings("unchecked") CallSummary parse(String raw){
  try{
   ...
   return new CallSummary(str(m,"shortSummary"),str(m,"fullSummary"),str(m,"topic"),str(m,"intent"),str(m,"sentiment"),str(m,"leadQuality"),str(m,"outcome"),str(m,"nextAction"),extracted instanceof Map<?,?> e?(Map<String,Object>)e:Map.of());
