...
class CrmSyncServiceTest {
    ...
    void existingPersonIsReusedAndCallNoteAndTaskAreSynchronized() {
        ...
        var call = new CrmCallData("external-1", "+994501234567", "700", Instant.now(), Instant.now(), "text", summary);
    ...
    private static final class RecordingGateway implements TwentyCrmGateway {
        final boolean existing; final AtomicInteger personCreates=new AtomicInteger(), upserts=new AtomicInteger(), notes=new AtomicInteger(), tasks=new AtomicInteger();
